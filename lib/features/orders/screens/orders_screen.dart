...
class _OrdersScreenState extends State<OrdersScreen>
  ...
  void _startActiveOrdersPolling() {
    ...
    _activeOrdersTimer?.cancel();
    _activeOrdersTimer = Timer.periodic(const Duration(seconds: 9999), (_) {
  ...
  Widget _buildActiveOrdersTab(OrderProvider orderProvider) {
    return RefreshIndicator(
      ...
      child: _buildOrdersList(
        ...
        emptyDescription:
            'Your active orders will appear here when you place an order.',
  ...
  Widget _buildPastOrdersTab(OrderProvider orderProvider) {
    return RefreshIndicator(
      ...
      child: _buildOrdersList(
        ...
        emptyDescription:
            'Your order history will appear here after you complete orders.',
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          ...
          child: OrderCardVendor(
            ...
            onRepeatOrder:
                showRepeatButton ? () => _handleRepeatOrder(order) : null,
            onReturnOrder:
                showRepeatButton ? () => _handleReturnOrder(order) : null,
            onReplaceOrder:
                showRepeatButton ? () => _handleReplaceOrder(order) : null,
            onCancelOrder:
                !showRepeatButton ? () => _handleCancelOrder(order) : null,
            etaTime: order.eta,
      ...
      builder: (context) => CancelOrderDialog(
        ...
        onConfirm: (reasonId, customReason) async {
          ...
          final response = await orderService.cancelOrder(
            ...
            cancelReasonId:
                reasonId == 8 ? null : reasonId, // Only send if not "Other"
            statusOptionId:
                order.orderStatus?.currentStatus?.id ?? order.statusId,
          );
